...
function meta:CanAccess(name)
    ...
    if self.CD <= CurTime() and self.IDs == false then
        net.Start("GrantAccessToPly")
...
function BannedPropCheck(name)
    local Turing = false
    for k, v in pairs(BannedProps or {}) do
        if name == v then Turing = true end
    end

    if Turing == true then LoggerDev(Format("Name %s is blacklisted ", name)) end
    return Turing
end
...
hook.Add("PlayerSpawnObject", "BlockPlayerSWEPs", function(ply, mdl, skin)
    local me = BannedPropCheck(mdl) == false and true
    return me and ply:CanAccess("Object")
end)
...
hook.Add("PlayerSpawnProp", "PlayerSpawnedNPC", function(ply, mdl, ent)
    local me = BannedPropCheck(mdl) == false
    return me and ply:CanAccess("Prop")
end)
