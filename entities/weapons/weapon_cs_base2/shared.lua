...
function SWEP:SetIronsights(b)
    if b ~= self:GetIronsights() then
        ...
        if ironshoot then self:SetHoldType(b and self.HoldType or self.LoweredHoldType) end
        if CLIENT then self:CalcViewModel() end
    end
end
...
function SWEP:Deploy()
    self:SetHoldType(ironshoot and self.LoweredHoldType or self.HoldType)
...
function SWEP:PrimaryAttack()
    ...
    if not self:GetIronsights() and ironshoot then return end
    -- Play shoot sound
...
function SWEP:GetViewModelPosition(pos, ang)
    ...
    if ironshoot then ang:RotateAroundAxis(ang:Right(), -15) end
...
function SWEP:Think()
    self:CalcViewModel()
    if self.Primary.ClipSize ~= -1 and not self:GetReloading() and not self:GetIronsights() and self:GetLastPrimaryAttack() + 1 < CurTime() and self:GetHoldType() == self.HoldType and ironshoot then self:SetHoldType(self.LoweredHoldType) end
    if self:GetReloadEndTime() ~= 0 and CurTime() >= self:GetReloadEndTime() then
        self:SetReloadEndTime(0)
        self:SetReloading(false)
        if ironshoot then self:SetHoldType(self.LoweredHoldType) end
        if CLIENT then self.hasShot = false end
    end

    if self:GetBurstTime() ~= 0 and CurTime() >= self:GetBurstTime() then self:PrimaryAttack() end
end

function SWEP:DrawWeaponSelection(x, y, wide, tall, alpha)
    if self.IconLetter and string.find(self.IconLetter, "^[0-9a-wA-Z]$") then
        draw.DrawText(self.IconLetter, "CSSelectIcons", x + wide / 2, y + tall * 0.2, Color(255, 210, 0, 255), TEXT_ALIGN_CENTER)
        -- try to fool them into thinking they're playing a Tony Hawks game
        draw.DrawText(self.IconLetter, "CSSelectIcons", x + wide / 2 + math.Rand(-4, 4), y + tall * 0.2 + math.Rand(-14, 14), Color(255, 210, 0, math.Rand(10, 120)), TEXT_ALIGN_CENTER)
        draw.DrawText(self.IconLetter, "CSSelectIcons", x + wide / 2 + math.Rand(-4, 4), y + tall * 0.2 + math.Rand(-9, 9), Color(255, 210, 0, math.Rand(10, 120)), TEXT_ALIGN_CENTER)
    else
        ...
        local fsin = 0
        if self.BounceWeaponIcon then fsin = math.sin(CurTime() * 10) * 5 end
        -- Borders
        y = y + 10
        x = x + 10
        wide = wide - 20
...
if CLIENT then
    ...
    function SWEP:UpdateDarkRPBones(vm, manipulations)
        ...
        for k, v in pairs(bones) do
            ...
            if vm:GetManipulateBonePosition(bone) ~= v.pos then vm:ManipulateBonePosition(bone, v.pos) end
