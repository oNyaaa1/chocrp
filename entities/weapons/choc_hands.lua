...
SWEP.Slot = 1
SWEP.SlotPos = 1
SWEP.Instructions = "Alt + E to deploy your fist, Right: (Deployed) Punch"
SWEP.UseHands = true
...
SWEP.HitDamage = {
    8, -- Normal punch damage
    12
}

SWEP.ComboDamage = {
    12, -- Combo punch damage
    24
}
...
function SWEP:Think()
    if self.SelectWepz == 1 then
        ...
        if SERVER and curtime > self:GetNextPrimaryFire() + self.ComboResetTime then self:SetCombo(0) end
    end

    local ply = self:GetOwner()
    if self.Thinkz < CurTime() and ply:KeyDown(IN_WALK) and ply:KeyDown(IN_USE) then
        if self.SelectWepz == 0 then
            self.SelectWepz = 1
            ply:SetRunSpeed(240)
        else
            self.SelectWepz = 0
        ...
        self.Thinkz = CurTime() + 1
    end
end

function SWEP:PrimaryAttack(right)
    if self.SelectWepz == 0 then return end
    local owner = self:GetOwner()
    owner:SetAnimation(PLAYER_ATTACK1)
    local anim = "fists_left"
    if right then anim = "fists_right" end
    if self:GetCombo() >= self.ComboCount then anim = "fists_uppercut" end
...
function SWEP:DealDamage()
    ...
    if IsValid(tr.Entity) then
        ...
        if IsValid(phys) then phys:ApplyForceOffset(owner:GetAimVector() * self.HitForceScale * phys:GetMass() * scale, tr.HitPos) end
    end
